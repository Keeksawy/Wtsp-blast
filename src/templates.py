...
def render_for_contact(kind, contact, v, follow_up_index=0, cfg=None):
    ...
    if kind == "initial":
        _overrides = db.get_settings().get("initialTemplateOverrides", {})
        _override_text = _overrides.get(template_obj["id"])
        if _override_text:
            template_obj = dict(template_obj, text=_override_text)
